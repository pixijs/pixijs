...
| `multiView`             | `boolean`                           | `false`     | Enable rendering to multiple canvases from one renderer (WebGL only; WebGPU always supports it). Pass each canvas as `target` in `renderer.render({ container, target })` — interaction events work on every canvas.     |
