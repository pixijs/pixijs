...
| `skipExtensionImports`         | `boolean` | `false` | Disable automatic import of default extensions. With `true`, import the subsystems you need manually (`import 'pixi.js/accessibility'`, `'pixi.js/app'`, `'pixi.js/events'`, …) and import any backend-specific modules a plugin would otherwise load through a `WebGLLoader`/`WebGPULoader`/`CanvasLoader` extension, since those loaders are skipped too. Used for custom tree-shaken builds. |
