...
export const scene: TestScene = {
    it: 'should center-align tagged split text correctly when words exceed wordWrapWidth',
    options: {
        width: 200,
        height: 420,
    },
...
function addBlock(scene: Container, y: number, str: string)
{
    ...
    const split = SplitText.from(text);

    split.pivot.set(split.width / 2, split.height / 2);
    split.position.set(100, y + 100);
    scene.addChild(split);
}
