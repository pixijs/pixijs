import { type GlTexture } from '../../../../src/rendering/renderers/gl/texture/GlTexture';
import { type GPUTextureGpuData } from '../../../../src/rendering/renderers/gpu/texture/GpuTextureSystem';
import { type WebGPURenderer } from '../../../../src/rendering/renderers/gpu/WebGPURenderer';
import { ExternalSource } from '../../../../src/rendering/renderers/shared/texture/sources/ExternalSource';
import { Texture } from '../../../../src/rendering/renderers/shared/texture/Texture';
import { Sprite } from '../../../../src/scene/sprite/Sprite';

import type { Renderer } from '../../../../src/rendering/renderers/types';
import type { Container } from '../../../../src/scene';
import type { TestScene } from '../../types';

// eslint-disable-next-line max-len
const base64Image = '';
// eslint-disable-next-line max-len
const bunnyBase64Image = ``;

export const scene: TestScene = {
    it: 'should render an external texture',
    create: async (scene: Container, renderer: Renderer) =>
    {
        const isWebGPU = !!((renderer as WebGPURenderer).gpu);

        const image = await new Promise((resolve, reject) =>
        {
            const img = new Image();

            img.onload = () => resolve(img);
            img.onerror = reject;
            img.crossOrigin = 'anonymous';

            img.src = `data:image/png;base64,${base64Image}`;
        });
        const bunnyImage = await new Promise((resolve, reject) =>
        {
            const img = new Image();

            img.onload = () => resolve(img);
            img.onerror = reject;
            img.crossOrigin = 'anonymous';

            img.src = `data:image/png;base64,${bunnyBase64Image}`;
        });

        const texture = Texture.from(image);
        const bunnyTexture = Texture.from(bunnyImage);

        // Use Pixi to load the images into the GPU
        renderer.texture.initSource(texture.source);
        renderer.texture.initSource(bunnyTexture.source);

        // Get the GPU texture and create an ExternalTexture from it
        const gpuData = texture.source._gpuData[renderer.uid];
        const gpuTexture = isWebGPU ? (gpuData as GPUTextureGpuData).gpuTexture : (gpuData as GlTexture).texture;
        const source = isWebGPU
            ? new ExternalSource({ resource: gpuTexture, renderer })
            : new ExternalSource({
                resource: gpuTexture,
                renderer,
                width: texture.source.width,
                height: texture.source.height,
            });
        const externalTexture = new Texture({
            source,
            dynamic: true,
        });
        const sprite = new Sprite(externalTexture);

        scene.addChild(sprite);

        // Simulate a render
        renderer.render({ container: scene });

        const bunnyGpuData = bunnyTexture.source._gpuData[renderer.uid];
        const bunnyGpuTexture = isWebGPU
            ? (bunnyGpuData as GPUTextureGpuData).gpuTexture : (bunnyGpuData as GlTexture).texture;

        // Use the new updateGPUTexture API
        // For WebGPU: dimensions are auto-detected from GPUTexture
        // For WebGL: dimensions must be provided (WebGLTexture is opaque)
        source.updateGPUTexture(
            bunnyGpuTexture,
            isWebGPU ? undefined : bunnyTexture.source.width,
            isWebGPU ? undefined : bunnyTexture.source.height
        );
    },
};
