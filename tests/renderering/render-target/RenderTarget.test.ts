...
describe('isRenderingToScreen', () =>
{
    ...
    it('should destroy', () =>
    {
        const renderTarget = setup({ width: 2, height: 1 });

        expect(renderTarget.colorTexture.source.resource).toBeInstanceOf(HTMLCanvasElement);
        expect(renderTarget.width).toBe(2);
        expect(renderTarget.height).toBe(1);

        renderTarget.destroy();

        expect(renderTarget.colorTexture).toBeUndefined();
        expect(renderTarget.colorTextures).toHaveLength(0);
        expect(renderTarget.width).toBe(0);
        expect(renderTarget.height).toBe(0);
    });
});
