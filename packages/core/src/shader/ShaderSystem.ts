...
export class ShaderSystem extends System
{
     ...
     * @param syncData - this is data that is passed to the sync function and any nested sync functions
     */
    syncUniformBufferGroup(group:UniformBufferGroup, name?: string, syncData?: any)
    {
        const glProgram = this.getGlProgram();
    ...
    syncUniformBuffer(uniformBufferGroup: UniformBufferGroup, glProgram: GLProgram, name:string, syncData: any): void
    {
        const syncFunc = uniformBufferGroup.syncUniforms || this.createSyncBufferGroup(uniformBufferGroup, name);

        this.renderer.buffer.bindBufferBase(uniformBufferGroup.buffer, syncData.uboCount++);

        // TODO wrap update in a cache??
        uniformBufferGroup.buffer.update();

        syncFunc(glProgram.uniformData,
            uniformBufferGroup.uniforms,
            this.renderer,
            defaultSyncData,
            uniformBufferGroup.buffer
        );
    }

    createSyncBufferGroup(uniformBufferGroup: UniformBufferGroup, name:string): UniformsSyncCallback
    {
        ...
        this.renderer.buffer.bind(uniformBufferGroup.buffer);

        const id = this.getSignature(uniformBufferGroup, this.shader.program.uniformData, 'ubo');

        if (!this.cache[id])
        {
            this.cache[id] = generateUniformBufferSync(uniformBufferGroup, this.shader.program.uniformData);
        }

        uniformBufferGroup.syncUniforms = this.cache[id];

        return uniformBufferGroup.syncUniforms;
    }

    /**
     * Takes a uniform group and data and generates a unique signature for them.
     *
     * @param {PIXI.UniformGroup | PIXI.UniformBufferGroup} group - the uniform group to get signature of
