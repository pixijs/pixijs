...
describe('PIXI.resources.ImageResource', function ()
{
    ...
    describe('premultiplyAlpha behaviour', function ()
    {
        before(function ()
        {
        ...
        it('should override BaseTexture premultiplyAlpha if specified', function ()
        {
            const image = new Image();
            const resource = new ImageResource(image, { autoLoad: false, premultiplyAlpha: 2 });
            ...
            return resource.load(false).then(() =>
            {
                ...
                expect(baseTexture.premultiplyAlpha).to.equal(2);
            });
        });

        it('should not override BaseTexture premultiplyAlpha if not specified', function ()
        {
            const image = new Image();
            const resource = new ImageResource(image, { autoLoad: false });
            const baseTexture = new BaseTexture(resource);

            baseTexture.premultiplyAlpha = 2;
            expect(resource.premultiplyAlpha).to.be.null;
