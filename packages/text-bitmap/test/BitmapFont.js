...
describe('BitmapFont', function ()
{
    describe('from', function ()
    {
        ...
        {
            expect(() => BitmapFont.from({ chars: 'CHARSET' })).to.throw;
        });
        ...
        {
            const font = BitmapFont.from({ name: 'foo', chars: [['a', 'z']] });
        ...
        {
            expect(() => BitmapFont.from({ name: 'foo', chars: [['l', 'i', 'm']] })).to.throw;
        });

        it('should throw an error when an invalid start/end of range', function ()
        {
            expect(() => BitmapFont.from({ name: 'foo', chars: [['z', 'a']] })).to.throw;
        });

        it('should render resolution with proportional size', function ()
        {
            const fontRes1 = BitmapFont.from({ name: 'foo', chars: 'a' });
            const fontRes2 = BitmapFont.from({ name: 'bar', chars: 'a', resolution: 2 });
