...
const rawFontStringNoBase = `<?xml version="1.0"?>
<font>
  <info face="ERAMv1_1" size="12" bold="0" italic="0" charset="" unicode="1" stretchH="100" smooth="0" aa="0" padding="0,0,0,0" spacing="0,0" outline="0"/>
  <common lineHeight="12" scaleW="1010" scaleH="12" pages="1" packed="0" alphaChnl="0" redChnl="4" greenChnl="4" blueChnl="4"/>
  <pages>
    <page id="0" file="eram-text-1.png" />
  </pages>
  <chars count="75">
...
describe('bitmapFontXMLStringParser', () =>
{
    ...
    it('should parse baseLineOffset from base', () =>
    {
        const parsedText = bitmapFontXMLStringParser.parse(rawFontString);

        expect(parsedText.baseLineOffset).toBe(1);
    });

    it('should default baseLineOffset to 0 when base is missing', () =>
    {
        const parsedText = bitmapFontXMLStringParser.parse(rawFontStringNoBase);

        expect(parsedText.baseLineOffset).toBe(0);
        expect(Number.isFinite(parsedText.baseLineOffset)).toBe(true);
    });
});
