...
| `transient`              | `boolean`                           | `false`     | _(WebGPU only, with `antialias`)_ Discard the canvas's multisample depth/stencil buffer at the end of each pass. Only for apps that never reopen the canvas pass while still needing depth or stencil: no filters or masks popping back onto it, no render with `clear: false`. |
