# pixi_split_text (Extension)

[< Back to nodes](../../gl2D-nodes.md) | [< Back to spec](../../gl2D-spec.md)

A PixiJS-specific node that splits text into individually transformable lines, words, and characters. The deserializer re-splits the text from the input parameters.
...
Requires `pixi_split_text` in `extensionsUsed` or `extensionsRequired`.
...
| webFont    | number \| string | Reference to a `web_font` resource                   |         | No       |
...
> **Note:** `resolution` and `webFont` are used during construction to configure text rendering before splitting. They do not map to properties on the `SplitText` class directly.
