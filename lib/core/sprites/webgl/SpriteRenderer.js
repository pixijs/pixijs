...
var SpriteRenderer = function (_ObjectRenderer) {
    ...
    function SpriteRenderer(renderer) {
        ...
        _this.indexBuffer = new _Buffer2.default(_this.indices, true, true);
    ...
    SpriteRenderer.prototype.contextChange = function contextChange() {
        ...
        for (var i = 0; i < this.vaoMax; i++) {
            var buffer = new _Buffer2.default(null, false);

            /* eslint-disable max-len */
            this.vaos[i] = new _Geometry2.default().addAttribute('aVertexPosition', buffer, 2, false, gl.FLOAT).addAttribute('aTextureCoord', buffer, 2, true, gl.UNSIGNED_SHORT).addAttribute('aColor', buffer, 4, true, gl.UNSIGNED_BYTE).addAttribute('aTextureId', buffer, 1, true, gl.FLOAT).addIndex(this.indexBuffer);
    ...
    SpriteRenderer.prototype.flush = function flush() {
        ...
        if (!_settings2.default.CAN_UPLOAD_SAME_BUFFER) {
            ...
            if (this.vaoMax <= this.vertexCount) {
                ...
                var _buffer = new _Buffer2.default(null, false);

                /* eslint-disable max-len */
                this.vaos[this.vertexCount] = new _Geometry2.default().addAttribute('aVertexPosition', _buffer, 2, false, gl.FLOAT).addAttribute('aTextureCoord', _buffer, 2, true, gl.UNSIGNED_SHORT).addAttribute('aColor', _buffer, 4, true, gl.UNSIGNED_BYTE).addAttribute('aTextureId', _buffer, 1, true, gl.FLOAT).addIndex(this.indexBuffer);
                /* eslint-enable max-len */

                this.vertexBuffers[this.vertexCount] = _buffer;
            }
